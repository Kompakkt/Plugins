...
const replaceStringInFile = async (path: string, search: string, replace: string) => {
...
const copyTemplatePlugin = async (pluginName: string) => {
  ...
  for (const entry of directories) {
    const targetPath = entry.parentPath.replace(source, target) + '/' + entry.name;
  ...
  for (const entry of files) {
    ...
    const targetPath =
      entry.parentPath.replace(source, target) + '/' + entry.name.replace('template', pluginName);
...
const modifyConfigFiles = async (pluginName: string) => {
  ...
  await replaceStringInFile(indexPath, 'template.plugin', `${pluginName}.plugin`);
...
const updateAngularConfig = async (pluginName: string) => {
  ...
  const stringified = JSON.stringify(angularConfig.projects['@kompakkt/plugin-template'], null, 2);
...
const main = async () => {
  ...
  const pluginName = await (async () => {
    ...
    if (name !== slugged) {
      ...
      if (!(await confirm(`Plugin name "${name}" will be changed to "${slugged}". Continue?`))) {
        console.log('Aborted');
        process.exit(0);
      }
    }
  ...
  console.log(`Success. You can now run "ng build ${packageName}" to build the plugin.`);
  process.exit(0);
};
